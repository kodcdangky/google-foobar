...
def solution(g):
    ...
    def valid_columns(previous_col_half, col_index):
        for config0 in cell_config_table[1]:
            if g[0][col_index] != kernel_sum_table[config0 + previous_col_half[0] + previous_col_half[1]]:
                continue
            col = [previous_col_half[0] + config0[:1], previous_col_half[1] + config0[1:]]
            col.extend(half for half in previous_col_half[2:])
            cell_stack = [(1, config) for config in cell_config_table[2]]
            while cell_stack:
                row_index, config = cell_stack.pop()
                if g[row_index][col_index] != kernel_sum_table[col[row_index] + col[row_index + 1][:1] + config]:
                    continue
