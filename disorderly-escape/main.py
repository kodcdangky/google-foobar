...
class Polynomial(list):
    ...
    E.g: f(x) = 30(x1)^3 + 20(x1)(x2) + 10(x3) + 1 => [[30, 3], [20, 1, 1], [10, 0, 0, 1], [1]]
    """
    ...
    def condense(poly):
        """
        Condenses a polynomial

        Args:
            poly (Polynomial)
        """
        # Walking backwards because removing items from a list left shift other elements,
        # starting with the 2nd last item
        for i, elem_i in zip(reversed(range(len(poly) - 1)), reversed(poly[:-1])):
            # Remove terms whose coefficient is 0
            if elem_i[0] == 0:
                poly.pop(i)
                continue

            # In each term, remove all trailing exponents of 0
            while elem_i[-1] == 0:
                elem_i.pop()

            # Walk backwards from the last element
            for j, elem_j in zip(reversed(range(len(poly))), reversed(poly[i + 1:])):
                # If their terms are the same, merge to the left term then delete the right term
    ...
    @staticmethod
    def sort(poly):
        """Sort a polynomial in descending order of exponents
...
def solution(w, h, s):
    ...
    def new_cycle_index_table(n: int) -> list[Polynomial]:
        """
    ...
    def weixu_product(poly1: Polynomial, poly2: Polynomial) -> list:
        """
    ...
    cycle_index_table = new_cycle_index_table(max(w, h))
    cycle_index_prod = weixu_product(cycle_index_table[w], cycle_index_table[h])
    return str(sum(coefficient * (s ** exponent) for coefficient, exponent in cycle_index_prod))
